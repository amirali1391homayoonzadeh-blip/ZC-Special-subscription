<?php
/** 
 * Plugin Name: ZC-Special-subscription
...
add_filter('the_content', function($content){
    ...
    return '<div class="zcss-restricted-content"><p>برای دسترسی به این محتوا نیاز به پلن سطح ' . esc_html($required_level) . ' یا بالاتر دارید. لطفاً پلن خود را ارتقا دهید.</p><a href="/upgrade-plan" class="button">ارتقای پلن</a></div>';
}, 10, 1);

/* -----------------------
   Admin UI: Plans manager (edit form includes duration_days and level)
   ----------------------- */
add_action('admin_menu', function(){
    add_menu_page('ZC Special Subscription', 'ZC Special Subscription', 'manage_options', 'zcss_plans', 'zcss_admin_page', 'dashicons-groups', 58);
});
...
function zcss_admin_page(){
    ...
    if(isset($_POST['zcss_action']) && check_admin_referer('zcss_admin_save','zcss_admin_nonce')){
        ...
        if($action==='save_plan'){
            ...
            echo '<div class="updated"><p>پلن ذخیره شد.</p></div>';
        }
        if($action==='delete_plan' && isset($_POST['del_slug'])){
            ...
            zcss_save_plans($new);
            echo '<div class="updated"><p>پلن حذف شد.</p></div>';
        }
        ...
        if($action==='assign_plan_to_user' && isset($_POST['user_id']) && isset($_POST['plan_slug'])){
            ...
            if($user_id > 0 && $plan_slug){
                ...
                if($result){
                    echo '<div class="updated"><p>پلن با موفقیت به کاربر اختصاص یافت.</p></div>';
                } else {
                    echo '<div class="error"><p>خطا در اختصاص پلن به کاربر.</p></div>';
                }
            } else {
                echo '<div class="error"><p>کاربر یا پلن معتبر نیست.</p></div>';
            }
        }
        if($action==='remove_user_plans' && isset($_POST['manage_user_id'])){
            ...
            if($removed_count > 0){
                echo '<div class="updated"><p>اشتراک‌های انتخاب‌شده حذف شدند.</p></div>';
            } else {
                echo '<div class="error"><p>هیچ اشتراکی حذف نشد.</p></div>';
            }
        }
        if($action === 'set_restriction' && isset($_POST['post_id']) && isset($_POST['required_level'])){
            ...
            if($post_id > 0){
                ...
                echo '<div class="updated"><p>محدودیت ذخیره شد.</p></div>';
            } else {
                echo '<div class="error"><p>آیتم معتبر نیست.</p></div>';
            }
        }
        ...
        if($action === 'delete_restriction' && isset($_POST['del_post_id'])){
            ...
            if($del_post_id > 0){
                ...
                echo '<div class="updated"><p>محدودیت حذف شد.</p></div>';
            } else {
                echo '<div class="error"><p>آیتم معتبر نیست.</p></div>';
            }
        }
    ...
    $plans = zcss_get_plans();
    $settings = get_option(ZCSS_OPTION_SETTINGS, array('auto_assign_free_on_login'=>1,'free_plan_slug'=>'basic-free','auto_create_wc_product'=>1));
    ...
    $selected_user_id = isset($_POST['manage_user_id']) ? intval($_POST['manage_user_id']) : 0;
    ...
    ?>
    <div class="wrap">
        <h1>ZC Special Subscription — مدیریت پلن‌ها</h1>
<h1>[zc_plans]</h1>
<h1>[zc_my_plans]</h1>
        <h2>تنظیمات</h2>
        <form method="post">
            ...
            <label><input type="checkbox" name="auto_assign_free_on_login" <?php checked(1, isset($settings['auto_assign_free_on_login']) ? $settings['auto_assign_free_on_login'] : 0); ?>> فعال‌سازی خودکار پلن رایگان هنگام اولین لاگین</label>
            <p>پلن رایگان پایه:
            <select name="free_plan_slug">
                <?php foreach($plans as $p): ?>
                    <option value="<?php echo esc_attr($p['slug']); ?>" <?php selected($settings['free_plan_slug'],$p['slug']); ?>><?php echo esc_html($p['title']); ?></option>
                <?php endforeach; ?>
            </select>
            </p>
            <label><input type="checkbox" name="auto_create_wc_product" <?php checked(1, isset($settings['auto_create_wc_product']) ? $settings['auto_create_wc_product'] : 1); ?>> اگر ووکامرس نصب است برای پلن‌های پولی محصول ایجاد شود</label>
            <p><button class="button button-primary">ذخیره تنظیمات</button></p>
        </form>

        <hr>
        <h2>پلن‌ها</h2>
        <table class="widefat"><thead><tr><th>عنوان</th><th>اسلاگ</th><th>قیمت</th><th>مدت (روز)</th><th>سطح</th><th>WC Product ID</th><th>عملیات</th></tr></thead><tbody>
        <?php if(empty($plans)): ?><tr><td colspan="7">پلنی تعریف نشده.</td></tr><?php else: foreach($plans as $p): ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo esc_url(add_query_arg('edit_slug', $p['slug'], admin_url('admin.php?page=zcss_plans'))); ?>" class="button">ویرایش</a>
                    <form method="post" style="display:inline">
                        ...
                        <button class="button" onclick="return confirm('حذف؟')">حذف</button>
                    </form>
                </td>
            </tr>
        ...
        <hr>
        <h2><?php echo $edit_plan ? 'ویرایش پلن' : 'افزودن پلن'; ?></h2>
        <form method="post">
            <?php wp_nonce_field('zcss_admin_save','zcss_admin_nonce'); ?>
            ...
            <table class="form-table">
                ...
                <tr><th>توضیحات</th><td><textarea name="description" rows="4" cols="50"><?php echo esc_textarea($edit_plan ? $edit_plan['description'] : ''); ?></textarea></td></tr>
                ...
                <tr><th>پولی است؟</th><td><label><input type="checkbox" name="is_paid" <?php checked(1, $edit_plan ? $edit_plan['is_paid'] : 0); ?>> بله</label></td></tr>
                <tr><th>مدت (بر حسب روز)</th><td><input type="number" name="duration_days" value="<?php echo esc_attr($edit_plan ? $edit_plan['duration_days'] : '365'); ?>"> <p class="description">0 = نامحدود. برای سالانه 365.</p></td></tr>
                <tr><th>سطح پلن</th><td><input type="number" name="level" value="<?php echo esc_attr($edit_plan ? $edit_plan['level'] : '1'); ?>"> <p class="description">عدد بالاتر = سطح بالاتر (مثال: 1 پایه، 2 حرفه‌ای).</p></td></tr>
            </table>
            <p><button class="button button-primary">ذخیره پلن</button></p>
        </form>
        ...
        <table class="widefat">
            ...
            <tbody>
                <?php if (empty($users_with_plans)): ?>
                    <tr><td colspan="6">هیچ کاربری با پلن یافت نشد.</td></tr>
                <?php else: ?>
                    <?php foreach ($users_with_plans as $user): ?>
                        <?php
                        ...
                        if (is_array($user_plans)) {
                            foreach ($user_plans as $slug => $info) {
                                ...
                                $plan_details[] = "$title (انقضا: $expires، وضعیت: $status، سطح: " . ($plan ? $plan['level'] : '-') . ")";
                            }
                        }
                        ?>
                        <tr>
                            ...
                            <td><?php echo $is_active ? 'فعال' : 'منقضی'; ?></td>
                ...
                <tr>
                    ...
                    <td>
                        <select name="plan_slug" required>
                            ...
                            <?php foreach($plans as $p): ?>
                                <option value="<?php echo esc_attr($p['slug']); ?>"><?php echo esc_html($p['title'] . ' (' . $p['slug'] . ' - سطح ' . $p['level'] . ')'); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                </tr>
            </table>
            <p><button class="button button-primary">اختصاص پلن</button></p>
        </form>
        ...
        <form method="post">
            ...
            <table class="form-table">
                <tr>
                    <th>انتخاب کاربر</th>
                    <td>
                        <select name="manage_user_id" required>
                            <option value="">-- انتخاب کاربر --</option>
                            <?php foreach($all_users as $user): ?>
            ...
            <p><button class="button button-primary">نمایش اشتراک‌ها</button></p>
        </form>
        <?php if ($selected_user_id > 0): ?>
            ...
            ?>
                <h3>اشتراک‌های کاربر: <?php echo esc_html($user->display_name); ?></h3>
                <form method="post">
                    <?php wp_nonce_field('zcss_admin_save','zcss_admin_nonce'); ?>
                    ...
                    <table class="widefat">
                        ...
                        <tbody>
                            <?php foreach ($user_plans as $slug => $info):
                                ...
                                $is_expired = !empty($info['expires']) && strtotime($info['expires']) < current_time('timestamp');
                                $status = $is_expired ? 'منقضی' : 'فعال';
                                $level = $plan ? esc_html($plan['level']) : '-';
                            ?>
                                <tr>
                    ...
                    <p><button class="button button-primary">حذف اشتراک‌های انتخاب‌شده</button></p>
                </form>
            <?php else: ?>
                <p>این کاربر هیچ اشتراکی ندارد.</p>
            <?php endif; ?>
        <?php endif; ?>
        <hr style="border: 2px solid #000; margin: 40px 0;">
        <h1>مدیریت محدودیت‌های دسترسی</h1>

        <hr>
        <h2>لیست محدودیت‌ها</h2>
        <table class="widefat"><thead><tr><th>شناسه محتوا</th><th>عنوان</th><th>نوع</th><th>سطح مورد نیاز</th><th>عملیات</th></tr></thead><tbody>
        <?php if(empty($restrictions)): ?><tr><td colspan="5">محدودیتی تعریف نشده.</td></tr><?php else: foreach($restrictions as $r): ?>
            <tr>
                ...
                <td>
                    <a href="<?php echo esc_url(add_query_arg('edit_restriction', $r['post_id'], admin_url('admin.php?page=zcss_plans'))); ?>" class="button">ویرایش</a>
                    <form method="post" style="display:inline">
                        <?php wp_nonce_field('zcss_admin_save','zcss_admin_nonce'); ?>
                        ...
                        <button class="button" onclick="return confirm('حذف محدودیت؟')">حذف</button>
                    </form>
                </td>
            </tr>
        <?php endforeach; endif; ?>
                ...
                <tr>
                    ...
                    <td>
                        <select name="post_type" required onchange="this.form.submit()">
                            ...
                            <option value="post" <?php selected('post', $selected_post_type); ?>>نوشته‌ها (Posts)</option>
                            <option value="page" <?php selected('page', $selected_post_type); ?>>برگه‌ها (Pages)</option>
                            <?php if(class_exists('WooCommerce')): ?>
                                <option value="product" <?php selected('product', $selected_post_type); ?>>محصولات (Products)</option>
                            <?php endif; ?>
                        </select>
                    </td>
                </tr>
        ...
        <?php if($selected_post_type): ?>
            <hr>
            <h2>انتخاب آیتم از <?php echo esc_html($selected_post_type); ?></h2>
            <form method="post">
                <?php wp_nonce_field('zcss_admin_save','zcss_admin_nonce'); ?>
                        ...
                        <td>
                            <select name="post_id" required onchange="this.form.submit()">
                                ...
                                <?php foreach($posts as $post): ?>
                                    <option value="<?php echo esc_attr($post->ID); ?>" <?php selected($selected_post_id, $post->ID); ?>><?php echo esc_html($post->post_title . ' (ID: ' . $post->ID . ')'); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </td>
                    </tr>
        ...
        <?php if($selected_post_id > 0): ?>
            <hr>
            <h2>تنظیم سطح مورد نیاز برای آیتم انتخاب‌شده</h2>
            <form method="post">
                <?php wp_nonce_field('zcss_admin_save','zcss_admin_nonce'); ?>
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">0 یا خالی = بدون محدودیت. عدد بالاتر = نیاز به سطح بالاتر.</p>
                        </td>
                    </tr>
                </table>
                <p><button class="button button-primary">ذخیره محدودیت</button></p>
            </form>
        <?php endif; ?>
    </div>
    <?php
...
add_shortcode('zc_my_plans', function(){
    ...
    if($user_plans && is_array($user_plans)){
        ...
        $out .= '<a href="/upgrade-plan" class="zcss-upgrade-btn">ارتقای پلن</a>';
        $out .= '</div>';
